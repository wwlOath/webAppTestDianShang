<template>
	<view class="app">
		...
		<view id="rating" class="rating-wrap column" :class="{'no-data': !ratingData.data}">
			...
			<rating-item v-if="JSON.stringify(ratingData) != '{}'" :item="ratingData"></rating-item>
		</view>
...
<script>
	...
	export default{
		...
		created() {
			...
			this.ratingList = data['mix-rating'].data;
		},
		// #endif
		methods:{
			async loadData(){
				...
				const data = this.productList.filter(item => item._id == this.id)[0];
			...
			async loadRating(){
				...
				*/
				...
				const data = this.ratingList.filter(item => item._id == this.id)[0];
