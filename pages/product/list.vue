...
<script>
	...
	export default {
		...
		created() {
			...
			this.productList = data['mix-product'].data;
			this.categroyProductList = data['mix-product-category'].data;
		},
		methods: {
			//加载产品列表
			async loadList(e){
				...
				this.list = [];
				const curList = this.productList.filter(item => item['cate_id'] == this.curCateItem._id)
				if(e.num === 1){
					//第一页清空数据重载
					this.list = [];
			...
			async loadCateList(){
				...
				//wwl add
				const data = this.categroyProductList.filter(item => item._id == this.curCateItem.parent_id);
				this.cateList = data[0].children.filter(item => item.parent_id == this.curCateItem.parent_id) || [];
				//wwl end
