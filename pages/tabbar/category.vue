	...
	<view class="app column">
		...
		<view class="content row">
			...
			<scroll-view class="right-side" scroll-y>
				...
				<view class="wrap">
					<view class="item column" v-for="item in current.children" :key="item._id" @click="navToList(item)">
	...
	export default {
		...
		methods: {
			async loadData(){
				...
				this.current = res[0];
				this.list = res; */
				this.current = this.categoryList[0];
			...
			navToList(item){
				const arr = [];
				this.current.children.forEach(cate=>{
					arr.push({
						_id: cate._id,
						name: cate.name
					})
